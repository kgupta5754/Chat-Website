#8e9998#8e9998# import required modules
import socket
import threading
...
def connect():
    ...
    username_label['text']= "Welcome " + username + " to our secure GLA_Chat"
...
top_frame = tk.Frame(root, width=600, height=100, bg=GREEN)
top_frame.grid(row=0, column=0, sticky=tk.NSEW)

middle_frame = tk.Frame(root, width=600, height=400, bg=MEDIUM_GREY)
middle_frame.grid(row=1, column=0, sticky=tk.NSEW)

bottom_frame = tk.Frame(root, width=600, height=100, bg=GREEN)
bottom_frame.grid(row=2, column=0, sticky=tk.NSEW)

username_label = tk.Label(top_frame, text="Enter your Name:", font=FONT, bg=GREEN, fg=BLACK)
username_label.pack(side=tk.LEFT, padx=10)

username_textbox = tk.Entry(top_frame, font=FONT, bg=WHITE, fg=BLACK, width=18)
username_textbox.pack(side=tk.LEFT)

username_button = tk.Button(top_frame, text="Enter GLA", font=BUTTON_FONT, bg=DARK_GREY, fg=WHITE, command=connect)
username_button.pack(side=tk.LEFT, padx=15)

message_textbox = tk.Entry(bottom_frame, font=FONT, bg=WHITE, fg=BLACK, width=38)
message_textbox.pack(side=tk.LEFT, padx=10)

message_button = tk.Button(bottom_frame, text="Send", font=BUTTON_FONT, bg=GREEN, fg=BLACK, command=send_message)
